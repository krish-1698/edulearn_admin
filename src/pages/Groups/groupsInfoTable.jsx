...
import GroupModal from "../../modal/groupsModal";
...
function GroupsInfoTable() {
  ...
  const displayedData = groupData.slice(startIndex, endIndex);

  useEffect(() => {
    Axios.get("http://localhost:3001/api/getAllGroups").then(
      (response) => {
  ...
  function deleteStudent(freelancerid) {
    ...
    Axios.delete(`http://localhost:3001/api/deleteGroup/${freelancerid}`).then((response) => {
      console.log("Nishaa Gopi");
      alert(response.data.message);
      window.location.reload(false);
    });
      ...
      <div style={{ marginLeft: "88%" }}>
        ...
        >
          ...
          <Box>
                     ...
                     <GroupModal setOpen={setOpen} open={open}  studentToEdit={editData}/>
                    </Box>
        </Box>
      </div>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          {row.imgPath}
                ...
                <TableCell ><img src={row.img_path} alt="Course Image" style={{ width: '50px', height: '50px' }} /></TableCell>
...
export default GroupsInfoTable;
